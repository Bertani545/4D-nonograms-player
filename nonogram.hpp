...
private:
	...
	*/
	...
	int Fix(int i, int j, vector<Color>& line, vector<int>& lineHints, vector<vector<int>>& dpTable) {
		...
		if (i == 0) {
			// We have hints left but no cells. 
			dpTable[i][j] = 0;
			return false;
		}
...
*/
		...
		//cout << "Nothig out of this one" << endl;
		return false;
	}
	...
	bool solve() {
		...
		if (isSolved()) {
			...
			this->unsolvedData = vector<Color>(this->data.size(), Color::Undefined);
			return true;
		}
		return false;
	}
...
/*
	void deleteCell(vector<int> cellIdx) {
		if (cellIdx.size() != this->numberDimensions) return;
		// obtain data id
		int idx = getCellId(cellIdx);
		if (this->unsolvedData[idx] == Color::Black) this->painted_squares--;
		this->unsolvedData[idx] = Color::Undefined;
	}
*/
	...
	vector<Color> transformIntToColorVec(vector<int>& int_vec) {
		vector<Color> res(int_vec.size());

		for (int i = 0; i < int_vec.size(); i++){
			res[i] = static_cast<Color>(int_vec[i]);
		}
	...
	// if a block exceeds not according to the hints, return all as unsolved
	vector<HintState> lineHintsArePlaced(struct LineDef& def) {
		...
		if (blackBlocks.size() == lineHints.size()) {
			...
			if (allSolved) {
				return vector<HintState>(lineHints.size(), HintState::solved);
			}
		}
	...
	bool userSolvedIt() {
		for (int i = 0; i < this->data.size(); i++) {
			...
			if (this->data[i] != this->unsolvedData[i]) return false;
		}
		return true;
	}
